...
"""
    build_explicit_observed_function(sys, ts; kwargs...)

Generate callable code that evaluates symbolic observed expression(s) `ts` for `sys`.
The expressions must be acyclic after expanding the system's observed equations.

# Arguments

- `sys::AbstractSystem`: The system that supplies unknowns, parameters, and observed
  equations. For input-output workflows, pass the `io_sys` returned by
  [`generate_control_function`](@ref); otherwise pass a completed system.
- `ts`: A symbolic expression or collection of expressions to evaluate. Symbols are resolved
  against `sys`; with `throw = true`, unknown symbols produce an error.

# Keyword Arguments

- `inputs = nothing`: Symbolic input variables included as an argument after the unknowns.
- `disturbance_inputs = nothing`: Unknown disturbance inputs. They are removed from the
  parameter arguments and are not function arguments.
- `known_disturbance_inputs = nothing`: Known disturbance inputs. They are removed from the
  parameter arguments and passed as the final function argument.
- `ps = parameters(sys; initial_parameters = true)`: Parameters supplied to the generated
  function, in argument order.
- `return_inplace = Val(false)`: For a collection of expressions, return both out-of-place
  and in-place callables. `Val(true)` and `Val(false)` avoid runtime dispatch.
- `param_only = false`: Generate a function that accepts only parameters, rather than
  unknowns and parameters.
- `expression = false`: Return generated `Expr` objects instead of callable functions.
- `eval_expression = false`: When generating callables, evaluate code in `eval_module`
  instead of constructing runtime-generated functions.
- `eval_module = @__MODULE__`: Module used when evaluating generated code.
- `output_type = Array`: Container type for vector-valued out-of-place results.
- `checkbounds = false`: Enable bounds checks while destructuring parameters. By default,
  generated code uses `@inbounds`.
- `throw = true`: Error when an expression references a symbol not present in `sys`.
- `wrap_delays = is_dde(sys) && !param_only`: Include a history-function argument and use it
  to evaluate delayed variables.
- `force_time_independent = false`: Omit the independent-variable argument for a
  time-dependent system.
- `compiler_options = CompilerOptions()`: Options controlling compilation of generated code.
- `kwargs...`: Options forwarded to `Symbolics.CodegenFunctionOptions`.

`disturbance_argument` is a deprecated compatibility keyword. Use
`known_disturbance_inputs` instead.

# Returns

Returns one callable for a scalar expression or, with `return_inplace = Val(true)`, a pair
`(f_oop, f_iip)` for a collection of expressions. The call arguments are ordered as output
buffer for the in-place form, unknowns unless `param_only`, `inputs` when supplied, parameter
buffer(s), the independent variable when present, and known disturbances when supplied.

With `expression = true`, returns the corresponding generated expression or expressions.
...
julia> @independent_variables t

julia> @variables x(t) u(t)

julia> @parameters k

julia> D = Differential(t);

julia> @named sys = System([D(x) ~ -k * (x + u)], t);

julia> (; io_sys) = generate_control_function(sys, [u]);

julia> g = build_explicit_observed_function(io_sys, x + u * t; inputs = [u]);

julia> g([1.0], [2.0], [2.0], 3.0)
7.0
```
"""
